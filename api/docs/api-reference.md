...
The `created_at` field reflects the creation date of the earliest task in the catalog.
...
- `catalog_id` — UUID of the Lightroom catalog, generated by the plugin
- `task_count` — number of tasks in this catalog
- `created_at` — creation timestamp of the earliest task in the catalog

Results are sorted by `created_at` ascending.

> **Note:** Catalogs with no tasks (e.g. after all tasks have been deleted) do not appear in this list.
